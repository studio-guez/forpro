...
class DBCollection(models.Model):
    ...
    def get_events(self, start_date: datetime, end_date: datetime) -> List:
        ...
        """
        cals = DBItem.objects.filter(collection=self)
        events = []

        for calendar in cals:
            cal = Calendar.from_ical(calendar.text)

            for event in cal.walk('vevent'):
                from agenda.models import Event
                event_date_start = event.get('dtstart').dt
                event_date_end = event.get('dtend').dt

                if not isinstance(event_date_start, datetime) and not isinstance(event_date_end, datetime):
                    continue
                else:
                    if event_date_start.date() >= start_date.date() and event_date_end.date() <= end_date.date():
                        e = namedtuple('Event', 'event_date start_date end_date start_time end_time title description')
                        e.event_date = event.get('dtstart').dt
                        e.start_date = event.get('dtstart').dt
                        e.end_date = event.get('dtend').dt
                        e.start_time = event.get('dtstart').dt.time()
                        e.end_time = event.get('dtend').dt.time()
                        e.title = event.get('summary')
                        e.description = event.get('location')
                        events.append(e)
