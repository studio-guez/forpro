<script lang="ts">
	...
	const copyByStatus: Record<number, ErrorCopy> = {
		...
		500: {
			...
			description: 'Une erreur inattendue est survenue de notre côté. Nous travaillons à la résoudre.'
		},
		502: {
			title: 'Service indisponible',
			description: 'Le site est momentanément indisponible. Merci de réessayer dans quelques instants.'
		},
		503: {
			title: 'Service indisponible',
			description: 'Le site est momentanément indisponible. Merci de réessayer dans quelques instants.'
...
<section class="max-w-360 mx-auto px-base pt-20 lg:pt-50 lg:grid lg:grid-cols-2 gap-x-8 gap-y-10">
    <div class="grid items-center lg:justify-center max-lg:mb-6">
		...
		<p class="col-start-1 row-start-1 text-center text-[15vw] lg:text-9xl font-bold text-green leading-none max-lg:max-w-1/2">{page.status}</p>
    </div>
	<div class="text-pink">
