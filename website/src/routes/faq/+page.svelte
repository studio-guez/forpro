<script lang="ts">
	...
	import type { FaqItem, TaxonomyTerm } from '$lib/interfaces/faq';
	...
	let selectedDomains = $state<string[]>(
		(initialParams.get('domains') ?? '').split(',').filter(Boolean)
	);
	...
	const activeDomains = $derived(
		selectedDomains.filter((slug) => usedTerms.some((term) => term.slug === slug))
	);
	...
	const matchesFilters = (faq: FaqItem): boolean => {
		if (
			...
			!faq.domains.some((term) => activeDomains.includes(term.slug))
		) {
			return false;
		}
	...
	const isFiltering = $derived(search.trim() !== '' || activeDomains.length > 0);
		...
		page.sections
			.map((section, index) => ({
				...
				commonTerms: section.faqs.length
					? section.faqs[0].domains.filter((term) =>
							section.faqs.every((faq) => faq.domains.some((t) => t.slug === term.slug))
						)
					: []
			}))
	...
	const toggleDomain = (slug: string): void => {
		selectedDomains = selectedDomains.includes(slug)
			? selectedDomains.filter((s) => s !== slug)
	...
	$effect(() => {
		...
		if (activeDomains.length > 0)
			parts.push(`domains=${encodeURIComponent(activeDomains.join(','))}`);
		...
		<fieldset class="mt-12 md:mt-18">
			...
			>
				{#each usedTerms as term (term.slug)}
					{@const selected = activeDomains.includes(term.slug)}
					<button
						type="button"
						...
						onclick={() => toggleDomain(term.slug)}
					>
						{term.title}
					</button>
				{/each}
