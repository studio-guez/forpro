		...
		<ul class="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12">
			{#each page.partners as partner, partnerIndex (partnerIndex)}
				<li>
					...
					<p class="text-body-2 font-bold">{partner.title}</p>
					...
					{#if partner.link}
						<a
							...
							class="text-body-2 flex items-center gap-2 mt-1 underline hover:opacity-50 transition-opacity"
						>
...
{#each page.sections as section, sectionIndex (sectionIndex)}
	<section aria-labelledby="impressum-section-{sectionIndex}" class="px-base {LABELLED_SECTION}">
		<h2 id="impressum-section-{sectionIndex}" class="text-body-2 font-bold">{section.title}</h2>

		<dl class={LABELLED_SECTION_ITEMS}>
			{#each section.credits as credit, creditIndex (creditIndex)}
				<div>
					<dt class="text-body-2 font-bold">{credit.role}</dt>

					{#each credit.names as name, nameIndex (nameIndex)}
						<dd class="text-body-2 mt-1">{name}</dd>
					{/each}
					...
					{#if credit.link}
						<dd class="mt-1">
							<a
								...
								target="_blank"
								rel="noopener noreferrer"
								class="text-body-2 flex items-center gap-2 underline hover:opacity-50 transition-opacity"
							>
								<IconLink class="shrink-0 w-4.5 h-4.5" />
								{credit.link.label}
							</a>
