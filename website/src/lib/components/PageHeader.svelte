<script lang="ts">
	...
	interface Props {
		...
		titleArrow?: ArrowColor | '' | null;
		cover?: CmsImage | null;
	}

	let { title, overtitle = null, titleArrow = null, cover = null }: Props = $props();
	...
	const arrowColorClass = $derived(titleArrow ? (arrowColors[titleArrow] ?? 'fill-white') : 'fill-white');
</script>
